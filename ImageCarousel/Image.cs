...
namespace ImageCarousel
{
    public class Image
    {
        ...
        public static void WriteImagesNamesInFile()
        {
            ...
            if (new FileInfo(HomeController.fileWithImagesListPath).Length == 0)
            { 
                for (int imageIndex = 0; imageIndex < HomeController.imagesToShow.Count; imageIndex++)
                {
                    using (StreamWriter streamWriter = new StreamWriter(HomeController.fileWithImagesListPath, false, System.Text.Encoding.Default))
                    {
        ...
        public static int GetRandomIndex()
        {
            ...
            {
                Random rand = new Random();
                int numberOfLinesInFile = File.ReadAllLines(HomeController.fileWithImagesListPath).Length-1;
        ...
        public static string GetRandomImageNameFromFile(int randomIndex)
        {
            ...
            using (StreamReader fileWithImagesList = new StreamReader(HomeController.fileWithImagesListPath))
            {
                randomImageName = ReadSpecificLine(HomeController.fileWithImagesListPath, randomIndex + 1);
            }
        ...
        {
            File.Delete(HomeController.fileWithImagesListPath);
        }
        ...
        {
            var fileForStoringImagesList = File.Create(HomeController.fileWithImagesListPath);
